...
export const createLeave = async(req, res) => {
    ...
    try {
        ...
        if(employee.isDeleted){
            return res.status(403).json({error: "You cannot create leave. Account has been deactivated"});
        }

        const {type, startDate, endDate, reason} = req.body;
        if(!type || !startDate || !endDate || !reason) return res.status(400).json({error: "Please fill in all fields"});
