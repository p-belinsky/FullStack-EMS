...
const autoCheckOut = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        if(!attendance?.checkOut){
            ...
            await step.sleepUntil("wait-for-the-1-hour", new Date(new Date().getTime() + 60 * 60 * 1000))
...
const leaveApplicationReminder = inngest.createFunction(
    ...
    async ({ event, step }) => {
       ...
       await step.sleepUntil("wait-for-the-1-day", new Date(new Date().getTime() + 24 * 60 * 60 * 1000))
